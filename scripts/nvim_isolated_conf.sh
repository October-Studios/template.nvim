...
\" Remember to ruin :PlugInstall after changing plugin section
...
if ! [ -z $LoadDirInput ]; then
  ...
  if [ -d $LoadDir ]; then
    ...
    export XDG_STATE_HOME=$NVIM_CONFIG_HOME/.local/share
    nvim $@
  else 
    echo "Sorry, cannot load neovim config. ${LoadDir} doesn't exist"
  fi 
